...
class AiMetadataValidationError extends Error {}
...
router.post('/metadata', requireAdmin, upload.single('file'), async (request, response) => {
  try {
    if (!request.file) return response.status(400).json({ success: false, error: '请选择一张图片' });
    return response.json({ success: true, data: await generateMetadata(request.file) });
  } catch (error: any) {
...
router.post('/metadata/photo', requireAdmin, async (request, response) => {
  try {
    const photoId = typeof request.body?.photoId === 'string' ? request.body.photoId.trim() : '';
    if (!photoId) return response.status(400).json({ success: false, error: '照片 ID 无效' });
    return response.json({ success: true, data: await generateMetadata(await getStoredPhotoFile(photoId)) });
  } catch (error: any) {
    const message = error instanceof Error ? error.message : 'AI 照片信息生成失败';
    console.error('AI 照片信息生成失败:', error);
    const status = error instanceof AiMetadataValidationError
